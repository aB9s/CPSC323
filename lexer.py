        ...
        if len(char) > 0:
            ...
            for c in char:
                ...
                if c == "!" and (token == "Unknown" or token == ""):
                    ...
                    token = ""
                # Double Operator
                elif token =="Operator" and c in DoubleOperators:
...
def print_to_file():
    ...
    while i < len(Token_Array):
        if Token_Array[i] != "":
            if Token_Array[i] == "Keyword" or Token_Array == "Unknown" or Token_Array[i] == "Integer" or Token_Array[
                i] == "Real":
                outputFile.write(
                    str(Line_Number_Array[i]) + "\t\t\t\t\t" + Token_Array[i] + "\t\t\t" + Lexeme_Array[i] + "\n")
